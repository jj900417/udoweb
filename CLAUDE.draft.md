...
2. **콘텐츠 접근 경로는 둘, 그리고 둘뿐이다.**
   ...
   번역이 안 걸린다 — 리뷰에서 잡는다.
3. **바뀌는 정보를 저장소에 복사하지 않는다.** 배 시간·요금·가게 영업시간·축제 날짜·
...
4. **앱 서버 호출은 항상 같은 출처 프록시로.** 브라우저가 `udo-info.fly.dev` 를 직접
...
9. **행정기관 공식 홈페이지로 표현하지 않는다.** 위탁·승인을 확인하기 전까지 "공식" 표현을
   쓰지 않는다(`index.html`·README·문안 전부).
10. **배포는 사람이.** `wrangler deploy` 를 자동 실행하지 않는다. 도메인
