...
- **글자 크기는 유틸로만**: `.t-display` `.t-section` `.t-body` `.t-meta`(15px) `.t-caption`(13px).
  페이지마다 다른 숫자를 쓰지 않는다. 캡션·credit 은 12px 아래로 내리지 않는다.
- **명조(`.display`, `.prose-archive`)는 표제와 역사·구술 본문에만.** 네비·버튼·메타데이터·
  실시간 정보는 고딕이다.
- **링크 색은 `text-link`, 버튼은 `bg-cta`** — 라이트모드에서 브랜드 시안(#00A5CD)은 글자
  대비가 모자란다(2.79:1). 시안은 테두리·인디케이터·틴트 배경 같은 **정체성 자리**에만 쓴다.
- **작품 이미지는 자르지 않는다.** `aspect-square object-cover` 는 `/gallery`(현재 사진 스트림)
  전용이다. 아카이브 작품은 원본 비율을 지킨다.
- **개발용 예시 데이터**(`src/archive/records/__dev__/`)는 `[예시]` 가 붙고 개발 서버에서만
  보인다. 실제 자료를 넣을 때 이 파일을 흉내 내지 말 것 — 거기엔 가짜 값이 들어 있다.
